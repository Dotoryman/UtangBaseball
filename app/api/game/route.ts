...
function calculateContact(error: number) {
  if (error > 0.34) return { outcome: 'WHIFF' as Outcome, distance: 0, exitVelocity: 0, launchAngle: 0, points: 0 };
  if (error > 0.24) return { outcome: 'FOUL' as Outcome, distance: 0, exitVelocity: 78, launchAngle: 48, points: 180 };
  const quality = clamp(1 - error / 0.24, 0, 1);
  const exitVelocity = Math.round(clamp(76 + quality * 87 + (secureRandom() - 0.5) * 8, 72, 166));
  const launchAngle = Math.round(clamp(-7 + quality * 37 + (secureRandom() - 0.5) * 10, -10, 38));
  const angleEfficiency = clamp(1 - Math.abs(launchAngle - 27) / 42, 0.2, 1);
  const distance = Math.round(clamp((exitVelocity - 68) * 1.42 * angleEfficiency + secureRandom() * 7, 8, 150));
...
function sessionId(body: Record<string, unknown>) {
  return typeof body.sessionId === 'string' && SESSION_ID.test(body.sessionId) ? body.sessionId : null;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!id || !Number.isInteger(pitchNumber) || pitchNumber < 1 || pitchNumber > TOTAL_PITCHES) {
      return Response.json({ error: '잘못된 경기 요청입니다.' }, { status: 400 });
    }
    const row = await env.DB.prepare('SELECT * FROM game_sessions WHERE id = ?').bind(id).first<SessionRow>();
    if (!row || row.completed_at) return Response.json({ error: '종료된 경기입니다.' }, { status: 409 });
    ...
    if (action === 'pitch') {
      ...
      if (!update.meta.changes) return Response.json({ error: '이미 시작한 공입니다.' }, { status: 409 });
    ...
    const updateStatement = env.DB.prepare(
      'UPDATE game_sessions SET contact_at = NULL, score = ?, combo = ?, max_combo = ?, home_runs = ?, max_distance = ?, completed_at = ? WHERE id = ? AND contact_at = ? AND completed_at IS NULL',
    ).bind(score, combo, maxCombo, homeRuns, maxDistance, completedAt, id, row.contact_at);
    ...
    console.error(JSON.stringify({ message: 'game request failed', error: error instanceof Error ? error.message : String(error) }));
    return Response.json({ error: '경기를 처리하지 못했습니다.' }, { status: 503 });
  }
}
