...
export default function Home() {
  ...
  return <main className="game-shell"><section className="phone-stage" aria-label="우땅야구 게임 화면">
    {screen === 'intro' && <div className="intro-panel screen-panel">
      ...
      <div className="intro-scene"><span className="intro-halo" aria-hidden="true" /><div className="intro-stickers" aria-hidden="true"><span className="sticker-bubble sticker-wave"><img src="/utang-pose-good-authentic.png" alt="" /></span><span className="sticker-bubble sticker-chill"><img src="/utang-pose-foul-authentic.png" alt="" /></span><span className="sticker-bubble sticker-ball"><img src="/utang-countdown-v7-cutout.png" alt="" /></span><span className="sticker-bubble sticker-sun"><img src="/utang-sun-logo.png" alt="" /></span></div><div className="hero-sprite" aria-hidden="true" /><span className="hero-spark spark-one">✦</span><span className="hero-spark spark-two">✦</span></div>
    ...
    {screen === 'playing' && <button type="button" className="play-field" onPointerDown={resolveSwing} aria-label="화면을 눌러 타격">
      ...
      <div className="stadium"><img src="/utang-stadium-v5.webp" alt="다양한 우땅이 관중들이 응원하는 야구장" className="stadium-background" /><span className="sr-only">투수 우땅이</span><div aria-hidden="true" className={`pitcher pitcher-${pitcherPhase}`}><span className="pitcher-sprite" style={{ backgroundPosition: showPitcherFollow ? '100% 0' : '0 0' }} /></div>{pitcherPhase === 'throw' && <span className="release-flash" aria-hidden="true" />}<div className="pitch-guide" aria-hidden="true" /><span className="sr-only">{showCatcherCatch ? '공을 잡은 포수 우땅이' : '포수 우땅이'}</span><div aria-hidden="true" className={`catcher catcher-${catcherPhase}`}><span className="catcher-sprite" style={{ backgroundPosition: showCatcherCatch ? '100% 0' : '0 0' }} /></div>
        ...
        {!countdown && !contact && <div className="swing-cue"><span className="tap-ring"><i /></span><strong>탭!</strong><small>SPACE</small></div>}{countdown && <div className="countdown-overlay" aria-live="assertive"><div className="countdown-card"><span className="countdown-kicker">UTANG BASEBALL</span><span className="countdown-friend countdown-friend-left" aria-hidden="true"><img src="/utang-pose-good-authentic.png" alt="" /></span><span className="countdown-friend countdown-friend-right" aria-hidden="true"><img src="/utang-pose-foul-authentic.png" alt="" /></span><div className="countdown-mascot"><span className="countdown-mascot-glow" aria-hidden="true" /><img src="/utang-countdown-v7-cutout.png" alt="야구공을 안고 준비하는 우땅이" /></div><small>9회말 · 10구 승부</small><strong key={countdown}>{countdown}</strong><b>{countdown === 'PLAY' ? 'PLAY BALL!' : '타격 준비'}</b><div className="countdown-dots" aria-hidden="true"><i className="active" /><i className={countdown === 2 || countdown === 1 || countdown === 'PLAY' ? 'active' : ''} /><i className={countdown === 1 || countdown === 'PLAY' ? 'active' : ''} /></div></div></div>}
      </div>
    ...
    {screen === 'result' && <div className="result-panel screen-panel"><header className="result-topbar"><button type="button" className="intro-brand brand-home" onClick={returnHome} aria-label="우땅야구 시작 화면"><img src="/utang-sun-logo.png" alt="햇님 우땅이" /><strong>우땅야구</strong></button></header><div className="result-emotes" aria-hidden="true"><span className="result-emote result-emote-wave"><img src="/utang-pose-good-authentic.png" alt="" /></span><span className="result-emote result-emote-chill"><img src="/utang-pose-foul-authentic.png" alt="" /></span><span className="result-emote result-emote-ball"><img src="/utang-countdown-v7-cutout.png" alt="" /></span><span className="result-emote result-emote-sun"><img src="/utang-sun-logo.png" alt="" /></span></div><p className="badge">경기 종료</p><div className="result-character"><span className="result-burst" /><img src={resultImage} alt="경기를 마친 우땅이" className={`result-image ${homeRuns > 0 ? 'result-image-homer' : score >= 5000 ? 'result-image-good' : 'result-image-miss'}`} /></div><p className="result-grade">{grade}</p><h2>{score.toLocaleString()}<small>점</small></h2><div className="result-stats"><div><span>홈런</span><strong>{homeRuns}개</strong></div><div><span>최고 비거리</span><strong>{maxDistance}m</strong></div><div><span>최고 콤보</span><strong>×{maxCombo}</strong></div></div>
