...
export default function Home() {
  ...
  return <main className="game-shell"><section className="phone-stage" aria-label="우땅야구 게임 화면">
    {screen === 'intro' && <div className="intro-panel screen-panel">
      ...
      {showHelp && <dialog open className="help-overlay" aria-label="우땅야구 소개"><div className="help-card about-card"><button type="button" className="help-close" onClick={() => setShowHelp(false)} aria-label="닫기"><X size={20} /></button><img src="/utang-sticker-clover-v091.png" alt="클로버를 든 우땅이" /><h2>우땅야구 만든 사람</h2><p className="about-intro">안녕하세요, 제작자 <b>dotoryman</b>입니다.<br />우땅이가 너무 귀여워서 우땅이 야구게임을 만들어봤어요. 즐겁게 플레이해보세요!</p><div className="about-notice"><p>우땅랜드와 마찬가지로 영리적인 목적의 페이지가 아닙니다.</p><p>우땅이 캐릭터의 저작권은 원작자 <b>소콘소콘</b>님께 있습니다.</p></div><p className="about-contact">개선 요청사항은 Instagram DM<br /><a href="https://www.instagram.com/dotoryman/" target="_blank" rel="noreferrer">@dotoryman</a>으로 보내주세요.</p><Button className="start-button about-thanks" onClick={() => setShowHelp(false)}>찾아줘서 고마어!</Button></div></dialog>}
    </div>}
