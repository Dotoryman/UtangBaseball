...
type JudgmentResponse = {
  ...
  stats: GameStats;
  completed: boolean;
  dailyBat?: ServerBatProgress | null;
};
type StartResponse = {
  sessionId?: string;
  batType?: BatType;
...
function calculateContact(error: number): Contact {
  ...
  const exitVelocity = Math.round(
    clamp(76 + quality * 87 + (Math.random() - 0.5) * 8, 72, 166),
  );
  const launchAngle = Math.round(
    clamp(-7 + quality * 37 + (Math.random() - 0.5) * 10, -10, 38),
  );
  const angleEfficiency = clamp(1 - Math.abs(launchAngle - 27) / 42, 0.2, 1);
  const distance = Math.round(
    clamp(
      (exitVelocity - 68) * 1.42 * angleEfficiency + Math.random() * 7,
      8,
      150,
    ),
  );
...
function koreanMidnight(now = Date.now()) {
...
function loadRecords(): RecordItem[] {
  ...
  try {
    const records = JSON.parse(
      localStorage.getItem('utang-baseball-records') ?? '[]',
    ) as RecordItem[];
    ...
    return records
      .filter(
        (record) =>
          ...
          record.playedAt >= koreanMidnight() &&
...
function loadDailyBatState(now = Date.now()): DailyBatState {
  ...
  try {
    return normalizeDailyBatState(
      JSON.parse(localStorage.getItem(DAILY_BAT_STORAGE_KEY) ?? 'null'),
      now,
    );
  } catch {
    return createDailyBatState(now);
  }
}
function saveDailyBatState(state: DailyBatState) {
  try {
    localStorage.setItem(DAILY_BAT_STORAGE_KEY, JSON.stringify(state));
  } catch {
    /* The game still works with a basic bat when storage is unavailable. */
  }
}
...
function triggerHitHaptic(outcome: Outcome) {
  if (
    ...
    return;
  navigator.vibrate(
    outcome === 'HOME_RUN' ? [32, 24, 52] : outcome === 'FOUL' ? 12 : 22,
  );
}
...
async function postGame<T>(body: Record<string, unknown>): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(data.error ?? '경기를 시작하지 못했어.');
...
async function createShareCard(
  ...
  try {
    ...
    const fallback = context.createLinearGradient(0, 0, 1200, 630);
    fallback.addColorStop(0, '#38251c');
    fallback.addColorStop(1, '#795039');
    context.fillStyle = fallback;
    context.fillRect(0, 0, 1200, 630);
  }
  context.textBaseline = 'alphabetic';
  context.textAlign = 'left';
  context.fillStyle = '#e3b85c';
  context.font = '900 23px Arial, sans-serif';
  context.fillText('UTANG BASEBALL · FINAL SCORE', 105, 154);
  const playerTitle = `${name} 선수의 야구 도전`;
  context.fillStyle = '#fff8e9';
  context.font = '900 42px "Malgun Gothic", sans-serif';
  if (context.measureText(playerTitle).width > 630)
    context.font = '900 34px "Malgun Gothic", sans-serif';
  context.fillText(playerTitle, 105, 211, 630);
  context.fillStyle = '#fff8e9';
  context.font = '900 98px "Arial Black", "Malgun Gothic", sans-serif';
  context.fillText(score.toLocaleString(), 100, 325);
  const scoreWidth = context.measureText(score.toLocaleString()).width;
  context.fillStyle = '#e3b85c';
  context.font = '900 34px "Malgun Gothic", sans-serif';
  context.fillText('점', 114 + scoreWidth, 321);
  context.strokeStyle = 'rgba(255,244,220,.22)';
  context.lineWidth = 2;
  context.beginPath();
  context.moveTo(105, 354);
  context.lineTo(735, 354);
  context.stroke();
  context.fillStyle = '#e7cba7';
  context.font = '800 20px "Malgun Gothic", sans-serif';
  context.fillText('홈런', 105, 397);
  context.fillText('최고 비거리', 310, 397);
  context.fillText('최고 콤보', 535, 397);
  context.fillStyle = '#fff8e9';
  context.font = '900 34px "Malgun Gothic", sans-serif';
  context.fillText(`${homeRuns}개`, 105, 443);
  context.fillText(`${maxDistance}m`, 310, 443);
  context.fillText(`×${maxCombo}`, 535, 443);
  context.fillStyle = '#e3b85c';
  context.font = '800 22px Arial, sans-serif';
  context.fillText('utangbaseball.cloud', 105, 515);
...
export default function Home() {
  ...
  const [pitcherPhase, setPitcherPhase] = useState<PitcherPhase>('idle');
  const [batterPhase, setBatterPhase] = useState<BatterPhase>('idle');
  const [catcherPhase, setCatcherPhase] = useState<CatcherPhase>('idle');
  ...
  const [activeBat, setActiveBat] = useState<BatType>('basic');
  const [batReward, setBatReward] = useState<BatReward | null>(null);
  const [dailyBatState, setDailyBatState] = useState<DailyBatState>(() =>
    createDailyBatState(),
  );
  const [networkNotice, setNetworkNotice] = useState('');
  const shareBusy = useRef(false);
  const startBusy = useRef(false);
  const shareCardRef = useRef<{ sessionId: string; cardId: string } | null>(
    null,
  );
  ...
  const pitchLockedRef = useRef(true);
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      setRecords((current) =>
        current.filter((record) => record.playedAt >= koreanMidnight()),
      );
      fetch('/api/scores?period=daily')
        ...
        .then((data) => {
          if (!cancelled && Array.isArray(data?.records))
            setRecords(
              data.records.filter(
                (record) => record.playedAt >= koreanMidnight(),
              ),
            );
        })
        .catch(() => undefined);
      ...
      timer = setTimeout(
        refresh,
        koreanMidnight() + 86400000 - Date.now() + 100,
      );
    };
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      setDailyBatState(state);
      clearTimeout(timer);
      timer = setTimeout(
        refresh,
        state.dayStart + 86_400_000 - Date.now() + 100,
      );
    };
    refresh();
    const onVisible = () => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setNetworkNotice(''), 3200);
  ...
  const finishGame = useCallback(
    (
      ...
      serverProgress?: ServerBatProgress | null,
    ) => {
      ...
      const nextRecords = [...loadRecords(), record]
        .sort((a, b) => b.score - a.score)
        .slice(0, 50);
      try {
      ...
      const completion = serverProgress
        ? {
            state: {
              dayStart: serverProgress.dayStart,
              completedGames: serverProgress.completedGames,
              equippedBat: serverProgress.equippedBat,
            },
            reward: serverProgress.reward,
          }
  ...
  const queuePitch = useCallback(
    async (nextNumber: number, runId = gameRunRef.current) => {
      ...
      if (sessionId) {
        try {
          const data = await postGame<{
            pitch?: { type: PitchType; duration: number };
          }>({ action: 'pitch', sessionId, pitchNumber: nextNumber });
          if (
            data?.pitch &&
            PITCHES.some(
              (item) =>
                item.type === data.pitch?.type &&
                item.duration === data.pitch?.duration,
            )
          )
            config = data.pitch;
          ...
          sessionReadyRef.current = Promise.resolve(null);
        }
      }
      if (runId !== gameRunRef.current) return;
      schedule(() => {
        ...
        setBatterPhase('ready');
        setBatterFrame(1);
        setCatcherPhase('prepare');
      }, 180);
      schedule(() => {
        if (runId !== gameRunRef.current) return;
        ...
        setPitcherPhase('throw');
        setPitch(nextPitch);
        const releaseSessionId = sessionRef.current;
        if (releaseSessionId) {
          releaseReadyRef.current = postGame<{ released?: boolean }>({
            action: 'release',
            sessionId: releaseSessionId,
            pitchNumber: nextNumber,
          })
            .then((data) => {
              if (!data.released) throw new Error('release-session');
              return true;
            })
            .catch(() => {
        ...
        schedule(() => {
          if (runId === gameRunRef.current) setPitcherPhase('followThrough');
        }, 300);
        schedule(async () => {
          ...
          let serverProgress: ServerBatProgress | null = null;
          if (sessionRef.current) {
            try {
              const data = await postGame<JudgmentResponse>({
                action: 'miss',
                sessionId: sessionRef.current,
                pitchNumber: nextNumber,
              });
              if (data?.contact && data.stats) {
                miss = data.contact;
                canonical = data.stats;
                serverProgress = data.dailyBat ?? null;
              } else throw new Error('miss-session');
            } catch {
              sessionRef.current = null;
              sessionReadyRef.current = Promise.resolve(null);
              setNetworkNotice(
                '서버가 잠깐 졸았어. 이번 기록은 기기에 남길게!',
              );
            }
          }
          if (runId !== gameRunRef.current) return;
          ...
          setCombo(0);
          setMaxCombo(current.maxCombo);
          setHomeRuns(current.homeRuns);
          setMaxDistance(current.maxDistance);
          setPitch(null);
          setContact(miss);
          setBatterPhase('followThrough');
          setCatcherPhase('catch');
          schedule(() => {
            if (runId === gameRunRef.current) setCatcherPhase('reaction');
          }, 300);
          schedule(() => {
            if (runId !== gameRunRef.current) return;
            ...
            else void queuePitch(nextNumber + 1, runId);
          }, 940);
  ...
  const startGame = useCallback(
    async (event?: { preventDefault(): void }) => {
      ...
      const wasResult = screen === 'result';
      trackEvent(wasResult ? 'retry_click' : 'play_click');
      if (!nickname.trim()) setNickname('우땅이');
      const playerName = nickname.trim() || '우땅이';
      let serverStart: StartResponse | null = null;
      try {
        serverStart = await postGame<StartResponse>({
          action: 'start',
          nickname: playerName,
        });
      } catch (error) {
        ...
        if (typeof status === 'number' && status < 500) {
          setNetworkNotice(
            error instanceof Error ? error.message : '닉네임을 확인해줘.',
          );
          startBusy.current = false;
          return;
        }
        setNetworkNotice('서버가 잠깐 졸았어. 이번 기록은 기기에 남길게!');
      }
      clearTimers();
      statsRef.current = { score: 0, homeRuns: 0, maxDistance: 0, maxCombo: 0 };
      ...
      if (
        serverStart?.batType &&
        typeof serverStart.completedGames === 'number' &&
        typeof serverStart.dayStart === 'number'
      ) {
        const synced = normalizeDailyBatState({
          dayStart: serverStart.dayStart,
          completedGames: serverStart.completedGames,
          equippedBat: serverStart.batType,
        });
      ...
      setMaxDistance(0);
      setShareNotice('');
      if (serverStart) setNetworkNotice('');
      setPitchNumber(0);
      setScreen('playing');
      setCountdown(3);
      schedule(() => {
        if (runId === gameRunRef.current) setCountdown(2);
      }, 700);
      schedule(() => {
        if (runId === gameRunRef.current) setCountdown(1);
      }, 1400);
      schedule(() => {
        if (runId === gameRunRef.current) setCountdown('PLAY');
      }, 2100);
      schedule(() => {
        if (runId !== gameRunRef.current) return;
        setCountdown(null);
        void queuePitch(1, runId);
      }, 2500);
      startBusy.current = false;
    },
    [clearTimers, nickname, queuePitch, schedule, screen],
  );
  const resolveSwing = useCallback(async () => {
    if (
      ...
      batterPhase === 'swing' ||
      contact ||
      pitchLockedRef.current
    )
      return;
    const runId = gameRunRef.current;
    pitchLockedRef.current = true;
    const swingStartedAt = performance.now();
    const swingElapsedMs = Math.round(
      clamp(swingStartedAt - pitch.startedAt, 0, pitch.duration * 1.25),
    );
    clearTimers();
    setBatterPhase('swing');
    setBatterFrame(2);
    setPitcherPhase('followThrough');
    ...
    [3, 4, 5, 6, 7].forEach((frame, index) =>
      schedule(() => {
        if (runId === gameRunRef.current) setBatterFrame(frame);
      }, [22, 48, 78, 114, 158][index]),
    );
    schedule(() => {
      if (runId !== gameRunRef.current) return;
      setBatterFrame(7);
    ...
    const progress = clamp(
      (performance.now() - pitch.startedAt) / pitch.duration,
      0,
      1.14,
    );
    let nextContact = calculateContact(
      measureVisualSwingError() ?? Math.abs(progress - CONTACT_PROGRESS),
    );
    const previewMakesContact = !['WHIFF', 'FOUL'].includes(
      nextContact.outcome,
    );
    let previewLaunched = false;
    let previewHaptic = false;
    if (previewMakesContact)
      schedule(() => {
        if (runId !== gameRunRef.current) return;
        previewLaunched = true;
        previewHaptic = true;
        triggerHitHaptic(nextContact.outcome);
        setPitch(null);
        setBallFlying(true);
    ...
    if (sessionRef.current) {
      try {
        ...
        if (data?.contact && data.stats) {
          nextContact = data.contact;
          canonical = data.stats;
          serverProgress = data.dailyBat ?? null;
        } else throw new Error('swing-session');
      } catch {
        sessionRef.current = null;
        sessionReadyRef.current = Promise.resolve(null);
        setNetworkNotice('서버가 잠깐 졸았어. 이번 기록은 기기에 남길게!');
    ...
    const contactFrameDelay =
      SWING_CONTACT_FRAME_MS - (performance.now() - swingStartedAt);
    if (contactFrameDelay > 0)
      await new Promise((resolve) =>
        window.setTimeout(resolve, contactFrameDelay),
      );
    if (runId !== gameRunRef.current) return;
    if (!previewHaptic) triggerHitHaptic(nextContact.outcome);
    const keepsCombo = !['WHIFF', 'FOUL'].includes(nextContact.outcome);
    const nextCombo = canonical?.combo ?? (keepsCombo ? combo + 1 : 0);
    const nextMaxCombo = canonical?.maxCombo ?? Math.max(maxCombo, nextCombo);
    const earned = calculateEarnedScore(
      nextContact.points,
      nextCombo,
      activeBat,
    );
    const nextScore = canonical?.score ?? score + earned;
    const nextHomeRuns =
      canonical?.homeRuns ??
      homeRuns + (nextContact.outcome === 'HOME_RUN' ? 1 : 0);
    const nextMaxDistance =
      canonical?.maxDistance ?? Math.max(maxDistance, nextContact.distance);
    statsRef.current = {
      score: nextScore,
      homeRuns: nextHomeRuns,
    ...
    setBallFlying(
      !isWhiff &&
        !isFoul &&
        (nextContact.outcome !== 'HOME_RUN' || previewLaunched),
    );
    setCatcherPhase(isWhiff ? 'prepare' : 'reaction');
    if (nextContact.outcome === 'HOME_RUN' && !previewLaunched)
      schedule(() => setBallFlying(true), 180);
    if (isWhiff)
      schedule(() => {
        setPitch(null);
        setCatcherPhase('catch');
      }, catchDelay);
    const finishDelay = isWhiff
      ? catchDelay + 900
      : nextContact.outcome === 'HOME_RUN'
        ? 2100
        : 1120;
    schedule(() => {
      if (runId !== gameRunRef.current) return;
      ...
      if (
        ...
        !(
          ...
          event.target.closest(
            'input, textarea, [contenteditable="true"], .hud-home',
          )
        )
      ) {
        event.preventDefault();
  ...
  const rankingPageCount = Math.max(
    1,
    Math.ceil(records.length / RANKING_PAGE_SIZE),
  );
  ...
  useEffect(() => {
    setRankingPage((page) => Math.min(page, rankingPageCount - 1));
  }, [rankingPageCount]);
  const grade =
    score >= 50000
      ? '전설의 우땅이'
  ...
  const shareScore = useCallback(async () => {
    ...
    try {
      ...
      let cardId =
        shareCardRef.current?.sessionId === sessionId
          ? shareCardRef.current.cardId
          : '';
      if (!cardId) {
        const upload = await fetch(
          `/api/share-card?session=${encodeURIComponent(sessionId)}`,
          {
            method: 'POST',
        ...
        const uploaded = upload.ok
          ? ((await upload.json()) as { id?: string })
          : null;
        if (!uploaded?.id) throw new Error('share-card-upload');
        cardId = uploaded.id;
        shareCardRef.current = { sessionId, cardId };
      }
      const query = new URLSearchParams({
        n: name,
        s: String(score),
        h: String(homeRuns),
        d: String(maxDistance),
        x: String(maxCombo),
        c: cardId,
      });
      ...
      if (typeof navigator.share === 'function') {
        setShareNotice('누구한테 자랑할까?');
        await navigator.share({ url: shareUrl });
        setShareNotice('자랑 완료!');
      } else {
        const copied = await copyText(shareUrl);
        setShareNotice(copied ? '자랑 링크 챙겼어!' : '링크를 못 챙겼어');
      }
    } catch (error) {
      if ((error as DOMException).name === 'AbortError')
        setShareNotice('다음에 자랑할래');
      else if (!cardUploaded) {
        setShareNotice('앗, 다시 눌러줘!');
      } else {
        const copied = await copyText(shareUrl);
        setShareNotice(copied ? '자랑 링크 챙겼어!' : '한 번만 더 눌러줘');
      }
    }
  ...
  const showPitcherFollow =
    pitcherPhase === 'throw' || pitcherPhase === 'followThrough';
  ...
  const umpireCall =
    contact?.outcome === 'WHIFF'
      ? !pitch
        ? 'strike'
        : 'idle'
      : contact && contact.outcome !== 'FOUL'
        ? 'fair'
        : 'idle';
  const umpire = (
    <div className={`umpire umpire-${umpireCall}`}>
    ...
    <main className="game-shell">
      <section className="phone-stage" aria-label="우땅야구 게임 화면">
          ...
          <div className="intro-panel screen-panel">
            ...
            <div className="intro-scene">
              ...
              <div className="intro-stickers" aria-hidden="true">
                <span className="sticker-bubble sticker-wave">
                  <img src="/utang-sticker-wave-v071.png" alt="" />
                </span>
                <span className="sticker-bubble sticker-chill">
                  <img src="/utang-sticker-chill-v071.png" alt="" />
                </span>
                <span className="sticker-bubble sticker-ball">
                  <img src="/utang-countdown-v071.png" alt="" />
                </span>
                <span className="sticker-bubble sticker-clover">
            ...
            <div className="intro-copy">
              <span className="intro-callout">공 온다!</span>
              <h1>
                우땅아, 잘 보고
                <br />
                <em>냅다 휘둘러!</em>
            ...
            <form className="nickname-form" onSubmit={startGame}>
              ...
              />
              <Button type="submit" className="start-button playful-action">
                <Play size={17} fill="currentColor" /> 일단 쳐보자!
              </Button>
            </form>
            ...
            <div className="intro-ranking">
              <div className="intro-ranking-head">
                <div className="ranking-title">
                  <Trophy size={17} /> 오늘 제일 잘 친 우땅이
                </div>
                <span>매일 00:00 · 5명씩</span>
              </div>
              {records.length > 0 ? (
              ...
              {rankingPageCount > 1 && (
                <div className="ranking-pagination">
                  <button
                    type="button"
                    onClick={() =>
                      setRankingPage((page) => Math.max(0, page - 1))
                    }
                  ...
                  <span>
                    {rankingPage + 1} / {rankingPageCount}
                  </span>
                  <button
                    type="button"
                    onClick={() =>
                      setRankingPage((page) =>
                        Math.min(rankingPageCount - 1, page + 1),
                      )
                    }
            ...
            <footer className="intro-footer">
              ...
              <i aria-hidden="true" /> <span>Made by Dotoryman</span>
            </footer>
            {showHelp && (
              <dialog open className="help-overlay" aria-label="우땅야구 소개">
                <div className="help-card about-card">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSecretTaps((count) => {
                        const next = count + 1;
                        if (next >= 5)
                          window.location.assign('/admin?from=secret');
                        return next;
                      })
                    }
                    aria-label="부시시 우땅이"
                  >
                    <img
                      src="/utang-sticker-messy-v093.png"
                      alt="이불을 들고 웃는 부시시 우땅이"
                    />
                  </button>
                  <h2>우땅야구 만든 사람</h2>
                  <p className="about-intro">
                    <span>
                      안녕하세요, 제작자 <b>dotoryman</b>입니다.
                    </span>
                    <br />
                    <span>우땅이가 너무 귀여워서</span>
                    <br />
                    <span>우땅이 야구게임을 만들어봤어요.</span>
                    <br />
                    <span>즐겁게 플레이해보세요!</span>
                  </p>
                  <div className="about-notice">
                    <p>
                      <span>우땅랜드와 마찬가지로</span>
                      <br />
                      <span>영리적인 목적의 페이지가 아닙니다.</span>
                    </p>
                    <p>
                      <span>우땅이 캐릭터의 저작권은</span>
                      <br />
                      <span>
                        원작자 <b>소콘소콘</b>님께 있습니다.
                      </span>
                    </p>
                  </div>
                  <p className="about-contact">
                    개선 요청사항은 Instagram DM
                    <br />
                    <a
                      href="https://www.instagram.com/dotoryman/"
                      target="_blank"
                      rel="noreferrer"
                    >
                      @dotoryman
                  ...
                  <Button
                    className="start-button about-thanks"
                    onClick={() => setShowHelp(false)}
                  >
                    찾아줘서 고마어!
                  </Button>
        ...
        {screen === 'playing' && (
          <button
            type="button"
            className={`play-field ${contact?.outcome === 'HOME_RUN' ? 'home-run-impact' : ''}`}
            onPointerDown={resolveSwing}
            aria-label="화면을 눌러 타격"
          >
            {contact?.outcome === 'HOME_RUN' && (
              <output className="homer-celebration">
                <strong>HOME RUN!</strong>
                <span className="homer-caption">
                  우땅이, 날렸다! · {contact.distance}m
                </span>
                <i aria-hidden="true" />
                <i aria-hidden="true" />
                <i aria-hidden="true" />
            ...
            <div className="arcade-board">
              <div className="arcade-board-main">
                ...
                <div className={`arcade-combo ${combo > 1 ? 'is-hot' : ''}`}>
                  <span>콤보</span>
                  <strong>
                    <Flame size={16} aria-hidden="true" />×{combo}
                  </strong>
                </div>
              </div>
              <div className="arcade-inning">
                <img
                  className="arcade-ball-icon"
                  src="/baseball-official-cutout.png"
                  alt="이번 공"
                />
                <div className="arcade-pitches" aria-hidden="true">
            ...
            <div className="stadium">
              ...
              <div
                aria-hidden="true"
                className={`pitcher pitcher-${pitcherPhase}`}
              >
                <span
                  className="pitcher-sprite"
                  style={{
                    backgroundPosition: showPitcherFollow ? '100% 0' : '0 0',
                  }}
                />
              </div>
              {pitcherPhase === 'throw' && (
                <span className="release-flash" aria-hidden="true" />
              )}
              <div className="pitch-guide" aria-hidden="true" />
              <span className="sr-only">
                {showCatcherCatch ? '공을 잡은 포수 우땅이' : '포수 우땅이'}
              </span>
              <div
                aria-hidden="true"
                className={`catcher catcher-${catcherPhase}`}
              >
                <span
                  className="catcher-sprite"
                  style={{
                    backgroundPosition: showCatcherCatch ? '100% 0' : '0 0',
                  }}
                />
              </div>
              {umpire}
              ...
              {!contact && (
                <div
                  className={`abs-zone ${pitch ? 'live' : ''}`}
                  aria-hidden="true"
                >
                  {Array.from({ length: 9 }, (_, index) => (
                    <i key={index} />
                  ))}
                  <b className="contact-core" />
                </div>
              )}
              {pitch && (
                <div
              ...
              {ballFlying && (
                <div
                  className={`flying-ball ${contact ? `flying-${contact.outcome.toLowerCase()}` : 'flying-preview'}`}
                >
                  <img src="/baseball-official-cutout.png" alt="" />
                </div>
              )}
              <div className={`batter-shadow batter-shadow-${batterPhase}`} />
              <div
                className={`batter batter-${batterPhase} ${contact ? `batter-result-${RESULT_META[contact.outcome].tier}` : ''}`}
              >
                <span className="sr-only">
                  {contact
                    ? `${RESULT_META[contact.outcome].label} 타격을 한 우땅이`
                    : '타격 준비 중인 우땅이'}
                </span>
                <span
                  className="batter-sprite-v6"
                  aria-hidden="true"
                  style={{
                    backgroundImage: `url(${BAT_SPRITES[activeBat]})`,
                    backgroundPosition: `${(batterFrame / (BATTER_FRAMES.length - 1)) * 100}% 0`,
                  }}
                />
                {contact && ['WHIFF', 'FOUL'].includes(contact.outcome) && (
                  <img
                    src={
                      contact.outcome === 'WHIFF'
              ...
              {!pitch && !contact && !countdown && (
                <div className="ready-label">투수 준비 중</div>
              )}
              {contact && (
                <div
                  className={`judgment judgment-${RESULT_META[contact.outcome].tier}`}
                >
                  <strong>{RESULT_META[contact.outcome].label}</strong>
                  {contact.distance > 0 && (
                    <span>
                      {contact.distance}m · {contact.exitVelocity}km/h
                    </span>
                  )}
                </div>
              )}
              {!countdown && !contact && (
                <div className="swing-cue">
              ...
              {countdown && (
                <div className="countdown-overlay" aria-live="assertive">
                  <div className="countdown-card">
                    <span className="countdown-kicker">UTANG BASEBALL</span>
                    <span
                    ...
                    <div className="countdown-mascot">
                      <span
                        className="countdown-mascot-glow"
                        aria-hidden="true"
                      />
                      <img
                        src="/utang-countdown-v071.png"
                        alt="야구공을 안고 준비하는 우땅이"
                      />
                    </div>
                    <small>우땅이의 야구 도전</small>
                    <strong key={countdown}>{countdown}</strong>
                    <b>{countdown === 'PLAY' ? 'PLAY BALL!' : '타격 준비'}</b>
        ...
        {screen === 'playing' && (
          <button
            type="button"
            className="hud-home arcade-home"
            aria-label="처음 화면으로"
            onClick={returnHome}
          >
            <HomeIcon size={19} />
        ...
        {screen === 'result' && (
          <div className="result-panel screen-panel">
            ...
            <div className="result-emotes" aria-hidden="true">
              <span className="result-emote result-emote-wave">
                <img src="/utang-sticker-wave-v071.png" alt="" />
              </span>
              <span className="result-emote result-emote-chill">
                <img src="/utang-sticker-chill-v071.png" alt="" />
              </span>
              <span className="result-emote result-emote-ball">
                <img src="/utang-countdown-v071.png" alt="" />
              </span>
              <span className="result-emote result-emote-sun result-emote-clover">
                <img src="/utang-sticker-clover-v091.png" alt="" />
              </span>
            </div>
            <p className="badge">다 쳤다!</p>
            <div className="result-character">
              <span className="result-burst" />
              <img
                src={resultImage}
                alt="경기를 마친 우땅이"
                className={`result-image ${homeRuns > 0 ? 'result-image-homer' : score >= 5000 ? 'result-image-good' : 'result-image-miss'}`}
              />
            </div>
            ...
            <p className="result-rank">
              오늘은 <strong>{rank || '-'}등!</strong> · 자정에 다시 시작
            </p>
            <div className="result-actions">
              <Button
                className="start-button result-retry"
                onClick={() => startGame()}
              >
                <RotateCcw size={18} /> 또 칠래!
              </Button>
              <Button
                className="share-button result-share"
                onClick={shareScore}
              >
                {shareNotice ? (
                  <Check size={18} />
              ...
              <Button
                variant="outline"
                className="home-button result-home"
                onClick={returnHome}
              >
                <HomeIcon size={18} /> 처음으로 갈래
              </Button>
            </div>
          </div>
        )}
        {screen === 'result' && batReward && (
          <dialog
            open
          ...
          >
            <div className={`bat-reward-card reward-${batReward}`}>
              <span
                className="reward-spark reward-spark-one"
                aria-hidden="true"
              >
                ✦
              </span>
              <span
                className="reward-spark reward-spark-two"
                aria-hidden="true"
              >
                ✦
              </span>
        ...
        {networkNotice && (
          <output className="network-notice" aria-live="polite">
            {networkNotice}
          </output>
        )}
      </section>
