...
const missAssets = [
  ...
  ...['aluminum', 'gold', 'ruby', 'diamond'].map((bat) => `public/utang-pose-miss-v121-${bat}.png`),
];
...
test('miss poses keep the dropped bat and lower body visibly opaque', async () => {
  for (const path of missAssets) {
    assert.ok((await opaqueRatio(path, { left: 0, top: 430, right: 310, bottom: 588 })) > 0.96, `${path} bat`);
    assert.ok((await opaqueRatio(path, { left: 220, top: 360, right: 525, bottom: 588 })) > 0.96, `${path} lower body`);
  }
});
