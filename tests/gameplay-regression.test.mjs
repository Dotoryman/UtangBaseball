...
const config = await readFile(
  new URL('../lib/game-config.ts', import.meta.url),
  'utf8',
);
...
test('v1.2 shares the seven-pitch faster game timing between client and server', () => {
  ...
  for (const source of [client, server])
    assert.match(
      source,
      /CONTACT_PROGRESS,\s+PITCHES,\s+TOTAL_PITCHES,\s+WINDUP_MS/,
    );
  assert.match(uiConfig, /SWING_CONTACT_FRAME_MS = 78/);
...
test('successful contact removes the incoming ball before showing the batted ball', () => {
  assert.match(
    client,
    /triggerHitHaptic\(nextContact\.outcome\);\s+setPitch\(null\);\s+setBallFlying\(true\)/,
  );
...
test('intro and result characters use the currently earned bat', () => {
  ...
  assert.match(
    client,
    /await preloadBatVisuals\(nextBat\)[\s\S]*?setScreen\('playing'\)/,
  );
});
...
test('hit vibration is attached to the existing 78ms contact frame', () => {
  assert.match(
    client,
    /previewHaptic = true;\s+triggerHitHaptic\(nextContact\.outcome\);\s+setPitch\(null\);\s+setBallFlying\(true\)/,
  );
  assert.match(
    client,
    /if \(!previewHaptic\)\s+triggerHitHaptic\(nextContact\.outcome\)/,
  );
});
...
test('server chooses the bat from completed scores and ignores a client bat field', () => {
  assert.match(
    server,
    /COUNT\(\*\) AS count FROM scores WHERE player_id = \? AND played_at >= \?/,
  );
