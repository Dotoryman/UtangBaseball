...
async function recolorBat(
  ...
  if (palette.gloss) {
    ...
    for (const point of component) {
      centerX += point % info.width;
      centerY += Math.floor(point / info.width);
    }
    ...
    let xy = 0;
    for (const point of component) {
      const dx = (point % info.width) - centerX;
      const dy = Math.floor(point / info.width) - centerY;
      xx += dx * dx;
      yy += dy * dy;
      xy += dx * dy;
    }
    ...
    for (const point of component) {
      const dx = (point % info.width) - centerX;
      const dy = Math.floor(point / info.width) - centerY;
      const u = dx * majorX + dy * majorY;
      const v = dx * minorX + dy * minorY;
  ...
  for (const point of component) {
    ...
    const lightness = Math.max(
      ...
      Math.min(
        1,
        (data[offset] + data[offset + 1] + data[offset + 2] - 430) / 330,
      ),
    );
    if (palette.mid) {
      ...
      const amount =
        highlight < 0.55 ? highlight / 0.55 : (highlight - 0.55) / 0.45;
      ...
      const u =
        (x - glossGeometry.centerX) * glossGeometry.majorX +
        (y - glossGeometry.centerY) * glossGeometry.majorY;
      const v =
        (x - glossGeometry.centerX) * glossGeometry.minorX +
        (y - glossGeometry.centerY) * glossGeometry.minorY;
      const uNorm =
        (u - glossGeometry.minU) /
        (glossGeometry.maxU - glossGeometry.minU || 1);
      const vNorm =
        (v - glossGeometry.minV) /
        (glossGeometry.maxV - glossGeometry.minV || 1);
      const stripe =
        uNorm > 0.12 && uNorm < 0.83 && vNorm > 0.64 && vNorm < 0.79;
      if (stripe) {
        const strength = 0.48 * Math.sin(((vNorm - 0.64) / 0.15) * Math.PI);
...
async function recolorGoldIcon(output, palette) {
  ...
  for (let offset = 0; offset < data.length; offset += 4) {
    ...
    if (a < 32 || r < 110 || g < 52 || g < b * 1.12 || r < g * 0.9) continue;
    const lightness = Math.max(0, Math.min(1, (r + g + b - 250) / 500));
    const pivot = lightness < 0.58 ? palette.dark : palette.mid;
    const target = lightness < 0.58 ? palette.mid : palette.light;
    const amount =
      lightness < 0.58 ? lightness / 0.58 : (lightness - 0.58) / 0.42;
...
async function removeGeneratedBackdrop(source, output) {
  ...
  const canRemove = (point) => {
    ...
    const b = data[offset + 2];
    return (
      Math.min(r, g, b) > 170 && Math.max(r, g, b) - Math.min(r, g, b) < 30
    );
  };
  const add = (point) => {
    if (!background[point] && canRemove(point)) {
      background[point] = 1;
      queue.push(point);
    }
  };
...
await Promise.all([
  removeGeneratedBackdrop(
    'assets/source/utang-bat-gold-v093-source.png',
    'public/utang-bat-gold-v093.png',
  ),
  removeGeneratedBackdrop(
    'assets/source/utang-bat-diamond-v093-source.png',
    'public/utang-bat-diamond-v093.png',
  ),
]);

const goldSparkles =
  Buffer.from(`<svg width="720" height="260" xmlns="http://www.w3.org/2000/svg">
...
for (const [variant, palette] of Object.entries(variants)) {
  ...
  await recolorBat(
    ...
    `old/public/utang-pose-miss-v093-${variant}.png`,
    [0, 360, 310, 590],
    palette,
  );
  const layers = await Promise.all(
    frameBuffers.map(async (frameBuffer, index) => ({
      input: await sharp(frameBuffer)
        .resize(320, 288, { fit: 'fill' })
        .png()
        .toBuffer(),
      left: index * 320,
      top: 0,
  ...
  await sharp({
    ...
    .composite(layers)
    .png({ compressionLevel: 9 })
    .toFile(`public/utang-batter-v8-${variant}-strip.png`);
}
